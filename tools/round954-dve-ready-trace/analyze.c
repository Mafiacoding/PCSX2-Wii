/* Round 954 (task #946 continuation): traces the two DVE/video-mode
 * state bytes Round 953 identified as the real open question -
 * 0x80022588 (gates 0x8000d670's DVE-ready retry loop: retries while
 * (byte & 0xFE) == 0x1C) and 0x80022618 (read by 0x8000bd58 to decide
 * which DVE-mode branch target to jump to, masked with 0xF0 against
 * 0x20/0x10/0x30/0x40) - across the full SCPH-50004 diskless boot, by
 * sampling both every 5,000,000-slice chunk (same chunking convention
 * as Rounds 951-953) and printing on any change. Also fully
 * disassembles 0x8000d520 (called by 0x8000d670 right before the
 * 0x80022588 check - the likely writer) and the 0x8000c1f8 branch
 * target 0x8000bd58 jumps to for the byte&0xF0==0x20 case (the most
 * common real DVE mode, matching Round 941's Circuit2 finding).
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
int main(int argc, char **argv)
{
    ...
    uint32_t last_588 = 0xFFFFFFFFu, last_618 = 0xFFFFFFFFu;
    int changes = 0;

    while (done < budget && !ee->halted) {
        system_run_interleaved(chunk);
        done += chunk;
        uint32_t v588 = ee_mem_read32(ee, 0x80022588u);
        uint32_t v618 = ee_mem_read32(ee, 0x80022618u);
        if (v588 != last_588 || v618 != last_618) {
            printf("[R954] slice=%llu ee_instr=%llu: [0x80022588]=0x%08x (was 0x%08x)  "
                   "[0x80022618]=0x%08x (was 0x%08x)\n",
                   (unsigned long long)done, (unsigned long long)ee->instructions_executed,
                   v588, last_588, v618, last_618);
            last_588 = v588; last_618 = v618;
            changes++;
        }
    }

    printf("\n[R954] FINAL: ee_instr=%llu total distinct-value changes observed=%d\n",
           (unsigned long long)ee->instructions_executed, changes);
    printf("[R954] FINAL: [0x80022588]=0x%08x  masked&0xFE=0x%02x  (retry-loop condition: ==0x1C -> %s)\n",
           last_588, last_588 & 0xFEu, ((last_588 & 0xFEu) == 0x1Cu) ? "TRUE (would retry)" : "false (would NOT retry)");
    printf("[R954] FINAL: [0x80022618]=0x%08x  masked&0xF0=0x%02x  (mode select: 0x20/0x10/0x30/0x40 known cases)\n",
           last_618, last_618 & 0xF0u);

    dump_function(ee, 0x8000d520u, 80, "0x8000d520 (called by 0x8000d670 before the 0x80022588 check)");
    dump_function(ee, 0x8000c1f8u, 80, "0x8000c1f8 (0x8000bd58's byte&0xF0==0x20 branch target)");

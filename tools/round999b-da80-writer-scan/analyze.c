...
int main(int argc, char **argv)
{
    ...
    /* Round 999b (task #978): MAJOR - found the real mechanism. 0x0026f8e0 is
     * a real inbound-event-packet handler: it polls a flag BYTE at the
     * address stored in struct+0 (0x0040DB58+0 = 0x2040DA80 uncached alias,
     * i.e. real RAM address 0x0040DA80), and if nonzero, copies a packet
     * from there into local stack space and dispatches buffer[event_id]
     * (our callback A/B) based on a field within that packet. So the real
     * outstanding question is: does ANYTHING ever write a nonzero byte to
     * EE RAM 0x0040DA80 (the real, cached form of the uncached alias
     * 0x2040DA80)? Scan for direct sb/sw MIPS stores constructing that
     * exact address, same methodology as Round 991's original 0x0040dc80
     * scan. */
    printf("\n[R999b] Static scan for lui 0x0041 / addiu ...,-9600 (0x0040DA80)\n"
           "constant-construction pattern across full 32MB RAM (Round 991 methodology,\n"
           "retargeted to the real inbound-packet address):\n");
    int hits = 0;
    for (uint32_t a = 0x00000000u; a < 0x02000000u - 16u; a += 4) {
        uint32_t w0 = ee_mem_read32(ee, a);
        if ((w0 >> 16) != 0x3c02u && (w0 >> 16) != 0x3c03u && (w0 >> 16) != 0x3c04u &&
            (w0 >> 16) != 0x3c05u && (w0 >> 16) != 0x3c06u && (w0 >> 16) != 0x3c07u)
            continue; /* lui $r2-7, imm */
        if ((int16_t)(w0 & 0xffff) != 0x0041) continue; /* lui $r, 0x0041 */
        for (uint32_t off = 4; off <= 12; off += 4) {
            uint32_t w1 = ee_mem_read32(ee, a + off);
            uint32_t op1 = (w1 >> 26) & 0x3f;
            if (op1 != 0x09 && op1 != 0x08) continue; /* addiu/addi */
            int16_t imm1 = (int16_t)(w1 & 0xffff);
            if (imm1 != -9600) continue; /* 0x0040DA80 = 0x00410000 - 9600 */
            hits++;
            printf("\n[R999b] HIT #%d at 0x%08x (lui) / 0x%08x (addiu -9600), context:\n",
                   hits, a, a + off);
            dump_range(ee, (a >= 24u) ? a - 24u : 0u, a + off + 40u);
            break;
        }
    }
    printf("\n[R999b] Total 0x0040DA80-construction hits: %d\n", hits);
    return 0;
}

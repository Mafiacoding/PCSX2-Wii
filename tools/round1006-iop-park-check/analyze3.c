/* Round 1007 (task #985, continuing Round 1006): Round 1006 found 3
 * real IOP HLE threads (tid=4/5/6, entries 0x0014a214/0x00151884/
 * 0x00151af0) sitting READY-but-never-scheduled because
 * iop_module_loader.c's is_loadcore_panic_loop() completion path sets
 * st->idle=1 directly, bypassing reschedule() entirely, while tid=1
 * (prio=64, the "synthetic bridge thread" per Round 513) stays marked
 * RUN forever. This tool identifies WHICH real IOP modules tid=4/5/6
 * belong to (via iop_module_loader.h's already-existing public
 * module-name/entry accessors - no new instrumentation), and
 * disassembles the code at tid=1's saved pc (0x0001a8b4, from Round
 * 1006's analyze2.c dump) to see what it was doing immediately before
 * falling into the panic-loop completion branch.
 *
 * The disassembler body below (disasm_one() and its helper tables) is
 * reused verbatim from tools/round1006-iop-park-check/analyze.c
 * (itself from round944, itself from round655's own R5900 table) -
 * safe for IOP/R3000A per that file's own citation (strict MIPS-I
 * subset).
 */
#include <stdio.h>
#include <stdlib.h>
...
static uint32_t iop_ram_word3(iop_state_t *iop, uint32_t addr)
{
...
static void disasm_window(iop_state_t *iop, uint32_t center, int words_before, int words_after, uint32_t mark)
{
    for (int32_t i = -words_before; i <= words_after; i++) {
        uint32_t addr = center + (uint32_t)(i * 4);
        uint32_t word = iop_ram_word3(iop, addr);
        char line[192];
        disasm_one(word, addr, line, sizeof(line));
        printf("%s0x%08X: %08X  %s\n", (addr == mark) ? ">>>" : "   ", addr, word, line);
    }
}

/* Identify which loaded IOP module (if any) an address falls within,
 * using iop_module_loader.h's own already-public per-module name/entry
 * accessors - no new bookkeeping, no guessing. */
static void identify_module(uint32_t addr)
{
    int n = iop_module_loader_get_module_count();
    uint32_t best_entry = 0;
    const char *best_name = NULL;
    for (int i = 0; i < n; i++) {
        uint32_t e = iop_module_loader_get_module_entry(i);
        const char *name = iop_module_loader_get_module_name(i);
        /* Modules load in ROMDIR order at increasing addresses; the
         * closest entry point AT OR BEFORE addr is the owning module
         * (same heuristic Round 513 already established/used). */
        if (e <= addr && e >= best_entry) { best_entry = e; best_name = name; }
    }
    if (best_name)
        printf("    -> nearest preceding module entry: \"%s\" @0x%08x (offset +0x%x)\n",
               best_name, best_entry, addr - best_entry);
    else
        printf("    -> no module entry found at/before this address\n");
}
...
int main(int argc, char **argv)
{
    ...
    printf("[R1007] reached ee_instr=%llu ee_pc=0x%08x iop_pc=0x%08x iop_idle=%d\n",
           (unsigned long long)ee->instructions_executed, ee->pc, iop->pc, iop->idle);

    int nmod = iop_module_loader_get_module_count();
    printf("\n[R1007] loaded module count=%d\n", nmod);
    for (int i = 0; i < nmod; i++) {
        printf("  [%2d] \"%s\" entry=0x%08x\n", i, iop_module_loader_get_module_name(i),
               iop_module_loader_get_module_entry(i));
    }

    uint32_t targets[4] = { 0x0001a8b4u, 0x0014a214u, 0x00151884u, 0x00151af0u };
    const char *labels[4] = { "tid=1 saved pc (RUN)", "tid=4 entry (READY)", "tid=5 entry (READY)", "tid=6 entry (READY)" };
    for (int i = 0; i < 4; i++) {
        printf("\n[R1007] === %s: 0x%08x ===\n", labels[i], targets[i]);
        identify_module(targets[i]);
        disasm_window(iop, targets[i], 8, 16, targets[i]);
    }

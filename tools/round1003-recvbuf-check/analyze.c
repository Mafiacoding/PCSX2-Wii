...
int main(int argc, char **argv)
{
    ...
    printf("\n[R1001] Dump this project's real bind-sid tracking table\n"
           "(sif_cmd_iop_dump_bind_table, Round 952) at the Round 990 resting point,\n"
           "to see which real RPC services have actually been bound:\n");
    uint32_t out_cd[8], out_sid[8];
    uint32_t cap = sif_cmd_iop_dump_bind_table(out_cd, out_sid, 8);
    printf("  table capacity=%u\n", cap);
    for (uint32_t i = 0; i < cap; i++) {
        printf("  slot %u: cd=0x%08x sid=0x%08x%s\n", i, out_cd[i], out_sid[i],
               out_sid[i] ? "" : "  (empty)");
    }
    printf("\n[R1001] rpc_bind_count=%llu\n",
           (unsigned long long)sif_cmd_iop_get_rpc_bind_count());
    printf("\n[R1003] sif_cmd_iop_get_ee_recvbuf()=0x%08x (compare to our target 0x0040DA80/0x2040DA80)\n"
           "         sif_cmd_iop_get_init_cmd_count()=%u\n",
           sif_cmd_iop_get_ee_recvbuf(), sif_cmd_iop_get_init_cmd_count());
    return 0;
}

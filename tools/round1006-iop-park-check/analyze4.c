/* Round 1008 (task #986, continuing Round 1007): resolve the two
 * open items Round 1007 left: (1) read back g.trampoline_addr's real
 * runtime value (via the new iop_module_loader_get_trampoline_addr()
 * accessor, added this round following the same established read-
 * only-diagnostic-accessor pattern as get_module_name()/get_module_
 * entry()) to settle whether the current freeze's pc (0x00155c00)
 * matches the trampoline-return idle=1 site found in Round 1007; and
 * (2) disassemble EESYNC's full module body (entry 0x00155aa0 through
 * well past the frozen pc) to determine whether it's still
 * legitimately mid-execution when the freeze hits, or has already
...
static uint32_t iop_ram_word4(iop_state_t *iop, uint32_t addr)
{
...
int main(int argc, char **argv)
{
    ...
    uint32_t tramp = iop_module_loader_get_trampoline_addr();
    printf("[R1008] reached ee_instr=%llu iop_pc=0x%08x iop_idle=%d trampoline_addr=0x%08x match=%s\n",
           (unsigned long long)ee->instructions_executed, iop->pc, iop->idle, tramp,
           (tramp == iop->pc) ? "YES-trampoline-reentry-site" : "no");

    printf("\n[R1008] EESYNC full module body disassembly (entry 0x00155aa0 through 0x00155c40):\n");
    for (uint32_t addr = 0x00155aa0u; addr <= 0x00155c40u; addr += 4) {
        uint32_t word = iop_ram_word4(iop, addr);
        char line[192];

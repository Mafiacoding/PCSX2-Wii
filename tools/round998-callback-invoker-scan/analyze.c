...
int main(int argc, char **argv)
{
    ...
    /* Round 998 (task #977): find where the buffer[N].field0/field8 function
     * pointers (callback A/B at 0x0026FEA8/0x0026FEB8, registered by Round 996)
     * get loaded and invoked via JALR. Scan for the pattern:
     *   lw   $rt, OFF($base)      ; OFF in {0,4,8,12,16,20,24,28} - one word of
     *                              ; a buffer[N] entry (stride 32 bytes, count=32
     *                              ; per Round 991)
     *   ... (0-3 instructions of delay-slot/setup) ...
     *   jalr $rt   (or jalr $rd, $rt)
     * across the full 32MB image. This is a real, evidence-driven callback-
     * dispatch pattern search - not a guess - since Round 996/997 already
     * confirmed the exact struct/buffer layout this would be reading from. */
    printf("\n[R998] Scanning for lw-then-JALR (indirect call) patterns that could be\n"
           "invoking the Round 996 callback pointers (offsets 0/8 within a 32-byte-\n"
           "stride buffer entry)...\n");
    int hits = 0;
    for (uint32_t a = 0x00000000u; a < 0x02000000u - 16u; a += 4) {
        uint32_t w0 = ee_mem_read32(ee, a);
        uint32_t op0 = (w0 >> 26) & 0x3f;
        if (op0 != 0x23) continue; /* lw */
        uint32_t rt = (w0 >> 16) & 0x1f;
        int16_t imm = (int16_t)(w0 & 0xffff);
        if (imm != 0 && imm != 8) continue; /* only field0/field8 loads */
        /* look ahead up to 4 instructions for jalr using $rt as target */
        for (uint32_t off = 4; off <= 16; off += 4) {
            uint32_t w1 = ee_mem_read32(ee, a + off);
            uint32_t op1 = (w1 >> 26) & 0x3f;
            uint32_t funct1 = w1 & 0x3f;
            uint32_t rs1 = (w1 >> 21) & 0x1f;
            if (op1 == 0x00 && funct1 == 0x09 && rs1 == rt) { /* jalr rd, rs=rt */
                hits++;
                printf("\n[R998] HIT #%d: lw offset=%d at 0x%08x, jalr at 0x%08x, context:\n",
                       hits, imm, a, a + off);
                dump_range(ee, (a >= 32u) ? a - 32u : 0u, a + off + 32u);
                break;
            }
        }
        if (hits >= 40) break;
    }
    printf("\n[R998] Total lw(off=0/8)-then-JALR hits (capped at 40): %d\n", hits);
    return 0;
}

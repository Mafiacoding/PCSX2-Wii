/* Round 950 (task #447/#536, user-directed pivot to SCPH-50004 v1.90
 * EUR BIOS): disassembles the EE resting loop found by the fresh
 * SCPH-50004 diskless boot survey (0x80007CD8-0x80007CF0 region,
 * reached organically before ee_instr=25,000,000, held flat through
 * 480,000,000 EE instructions) plus dumps IOP module-loader stats and
 * CDVD/SIO2 state, for comparison against the SCPH-10000 baseline
 * documented in Rounds 943-949. Read-only diagnostic, no source
 * changed. disasm_one() below is the same verbatim-reused block
 * originally from tools/round655-ee-disasm/disasm.c.
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
int main(int argc, char **argv)
{
    ...
    printf("ee_pc=0x%08x iop_pc=0x%08x\n", ee->pc, iop->pc);

    printf("\n=== EE disasm 0x80007CA0-0x80007D20 ===\n");
    for (uint32_t a = 0x80007CA0u; a <= 0x80007D20u; a += 4) {
    ...
    printf("\n=== IOP module loader stats ===\n");
    int mc = iop_module_loader_get_module_count();
    printf("module_count=%d\n", mc);
    for (int i = 0; i < mc && i < 40; i++)
        printf("  [%2d] %-12s entry=0x%08x\n", i, iop_module_loader_get_module_name(i),
               iop_module_loader_get_module_entry(i));

    printf("\n=== CDVD / SIO2 state ===\n");
    printf("cdvd_status=0x%02x cdvd_ready=0x%02x scmd_call_count=%llu\n",
           iop_cdvd_get_status(), iop_cdvd_get_ready(),
           (unsigned long long)iop_cdvd_get_scmd_call_count());
    printf("sio2_pad_command_count=%u\n", iop_sio2_get_pad_command_count());

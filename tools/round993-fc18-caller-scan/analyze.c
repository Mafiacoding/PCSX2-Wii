...
int main(int argc, char **argv)
{
    ...
    printf("\n[R993] Scanning full 32MB RAM for direct 'jal 0x%08x' (encoding 0x%08x)...\n",
           TARGET_FUNC, JAL_TARGET_ENC);
    int hits = 0;
    for (uint32_t a = 0x00000000u; a < 0x02000000u; a += 4) {
        uint32_t w = ee_mem_read32(ee, a);
        if (w != JAL_TARGET_ENC) continue;
        hits++;
        printf("\n[R993] HIT #%d: jal 0x%08x at 0x%08x, wide context:\n", hits, TARGET_FUNC, a);
        dump_range(ee, (a >= 48u) ? a - 48u : 0u, a + 48u);
    }
    printf("\n[R993] Total direct-JAL hits: %d\n", hits);

    printf("\n[R993] Guard flag MEM[0x002ab284] (0=never called this init, 1=already ran): 0x%08x\n",
           ee_mem_read32(ee, 0x002ab284u));
    printf("\n[R993] Companion struct 0x0040DB58-0x0040DB78 (all-zero = init never actually ran; "
           "populated = it did):\n");
    dump_range(ee, 0x0040db58u, 0x0040db78u);

    printf("\n[R993] Continuation path (what runs when guard already set, 0x0026f818-0x0026f8a0):\n");
    dump_range(ee, 0x0026f818u, 0x0026f8a0u);

    if (hits == 0) {
        /* No direct JAL found - the call may be indirect (JALR through a
         * register holding the address, e.g. a function-pointer table
         * entry). Scan for any JALR whose target register was recently
         * loaded with 0x0026fc18 via the same lui/addiu-style constant
         * construction used elsewhere in this project's own compiled
         * output, within a short lookahead window. */
        printf("\n[R993] No direct JAL found - scanning for lui/addiu(0x0026fc18) constant "
               "construction anywhere (possible indirect JALR caller)...\n");
        int hits2 = 0;
        for (uint32_t a = 0x00000000u; a < 0x02000000u; a += 4) {
            ...
            if (op0 != 0x0F || imm0 != 0x0027) continue; /* lui $rt0, 0x0027 (0x0026fc18's upper half) */
            for (uint32_t off = 4; off <= 16; off += 4) {
                ...
                if (op1 != 0x09) continue;
                if (rs1 != rt0 || rt1 != rt0) continue;
                if (imm1 != (int16_t)0xfc18) continue;
                hits2++;
                printf("\n[R993] Indirect-construction HIT #%d: lui at 0x%08x, addiu at 0x%08x "
                       "(register $%s):\n", hits2, a, a + off, reg_name(rt0));
                dump_range(ee, a, a + off + 32);
                break;
            }
        }
        printf("\n[R993] Total indirect-construction hits: %d\n", hits2);
    }

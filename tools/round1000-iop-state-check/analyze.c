...
int main(int argc, char **argv)
{
    ...
    printf("\n[R1000] IOP state at the Round 990 resting point:\n");
    iop_state_t *iop = iop_core_get_state();
    printf("  IOP pc=0x%08x halted=%d instructions_executed=%llu\n",
           iop->pc, iop->halted, (unsigned long long)iop->instructions_executed);
    printf("\n[R1000] IOP code context (real disasm around current pc):\n");
    for (uint32_t a = (iop->pc >= 64u) ? iop->pc - 64u : 0u; a <= iop->pc + 64u; a += 4) {
        uint32_t w = iop_mem_read32(iop, a);
        char buf[128];
        disasm_one(a, w, buf, sizeof(buf));
        printf("  0x%08x: %08x  %s%s\n", a, w, buf, (a == iop->pc) ? "  <-- IOP PC" : "");
    }
    return 0;
}

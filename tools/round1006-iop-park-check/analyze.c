...
/* Round 1006 (task #984, continuing Round 1005): Round 1005 found the
 * EE genuinely WaitSema(0)-parked (real scheduler-confirmed, semid=0
 * already SignalSema'd 19 times this boot) and noted the IOP's own pc
 * (0x00155C00) reads identically across every 1M-instruction chunk
 * boundary sampled, despite system_run_interleaved() calling
 * iop_core_step() unconditionally every slice (not gated on the EE's
 * idle state). This tool distinguishes "genuine IOP busy-park" from
 * "sampling-granularity coincidence" directly: boot fresh from
 * instruction 0 to the same point, then step the IOP ULTRA fine-
 * grained (single ee_core_step()-equivalent iop steps, no 1M chunking)
 * for a further 2000 raw instructions, logging pc every step. If pc
 * genuinely never changes, that's conclusive proof of a real park (not
 * a coincidence of chunk-boundary sampling) - and the disassembly dump
 * below shows exactly what real IOP instruction it's parked on.
 */
...
static uint32_t iop_ram_word2(iop_state_t *iop, uint32_t addr)
{
...
int main(int argc, char **argv)
{
    ...
    if (argc < 3) { fprintf(stderr, "usage: %s <bios_path> <budget>\n", argv[0]); return 1; }
    bios_image_t bios;
    ...
    printf("\n[R1006] arrived: ee_instr=%llu ee_pc=0x%08x iop_instr=%llu iop_pc=0x%08x iop_halted=%d iop_idle=%d\n",
           (unsigned long long)ee->instructions_executed, ee->pc,
           (unsigned long long)iop->instructions_executed, iop->pc, iop->halted, iop->idle);

    /* Fine-grained park-vs-coincidence check: step ONLY the IOP core
     * (bypassing the EE/interleave ratio entirely) for 2000 raw steps,
     * logging pc/instr every single step. */
    printf("\n[R1006] fine-grained IOP-only stepping (2000 raw iop_core_step() calls):\n");
    uint32_t last_pc = iop->pc;
    uint64_t last_instr = iop->instructions_executed;
    int changed_count = 0;
    for (int i = 0; i < 2000; i++) {
        if (iop->halted) { printf("  [i=%d] IOP halted mid-check\n", i); break; }
        iop_core_step();
        if (iop->pc != last_pc || iop->instructions_executed != last_instr) {
            changed_count++;
            if (changed_count <= 20) {
                printf("  [i=%d] pc changed: 0x%08x -> 0x%08x  instr: %llu -> %llu\n",
                       i, last_pc, iop->pc, (unsigned long long)last_instr, (unsigned long long)iop->instructions_executed);
            }
            last_pc = iop->pc;
            last_instr = iop->instructions_executed;
        }
    }
    printf("[R1006] total pc/instr changes across 2000 raw steps: %d\n", changed_count);
    printf("[R1006] final after fine-grained check: iop_pc=0x%08x iop_instr=%llu\n",
           iop->pc, (unsigned long long)iop->instructions_executed);

    printf("\n[R1006] IOP GPR dump at parked pc:\n");
    static const char *names[32] = {
        "zero","at","v0","v1","a0","a1","a2","a3",
        "t0","t1","t2","t3","t4","t5","t6","t7",
        "s0","s1","s2","s3","s4","s5","s6","s7",
    ...
    printf("[R1006] pc=0x%08x next_pc=0x%08x hi=0x%08x lo=0x%08x\n", iop->pc, iop->next_pc, iop->hi, iop->lo);
    printf("[R1006] COP0 Status=0x%08x Cause=0x%08x EPC=0x%08x\n", iop->cop0[12], iop->cop0[13], iop->cop0[14]);

    printf("\n[R1006] Disassembly around parked pc 0x%08x:\n", iop->pc);
    for (uint32_t addr = iop->pc - 32u; addr <= iop->pc + 32u; addr += 4) {
        uint32_t word = iop_ram_word2(iop, addr);
        char line[192];

...
int main(void) {
    ...
    /* --- Test 3: unsupported tag ID flags an error instead of misbehaving --- */
    g_sink_capture_len = 0; g_sink_calls = 0;
    memset(g_ram, 0, sizeof(g_ram));
    wle32(g_ram + 0x4000, (DMA_TAG_REF << 28) | 1u); /* REF - not implemented */
    wle32(g_ram + 0x4004, 0x5000u);

    g_dma.chan[DMA_CHANNEL_GIF].tadr = 0x4000;
    dma_mmio_write32(0x1000A000u, 0x00000104u);

    CHECK(dma_get_state()->chan[DMA_CHANNEL_GIF].last_error == DMA_ERR_UNSUPPORTED_TAG, "unsupported tag ID (REF) sets last_error instead of silently mishandling it");

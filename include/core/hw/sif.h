...
#define SIF_SID_MTAP_PORT_OPEN  0x80000901u /* real, ee/rpc/multitap/src/libmtap.c "#define MTAPSERV_PORT_OPEN 0x80000901", bound by mtapInit() via sceSifBindRpc(&clientPortOpen, MTAPSERV_PORT_OPEN, 0); matching real IOP side iop/sio/mtapman/src/rpcservers.c "sceSifRegisterRpc(&sd1, MTAPSERV_PORT_OPEN, (void*)RpcServerPortOpen, sb, 0, 0, &qd1)" whose handler calls the real iop/sio/mtapman/src/freemtap.c mtapPortOpen(u32 port). Real client stub mtapPortOpen(int port) sends a single 4-byte word (the port number) as fno=1 and reads back an 8-byte reply (word[0]=echoed/unchanged since the real IOP handler only writes data[1], word[1]=actual s32 result). Both real source files retrieved from the full ps2sdk-master.zip tree (broader than this project's narrower docs/reference/ps2sdk/ee/kernel subtree) - Round 1038/1039, identified via a correlated dual-trace experiment as the exact real service immediately preceding the SCPH-50004 diskless boot's previously-stuck 20th completion registration. */
#define SIF_SID_MTAP_PORT_CLOSE 0x80000902u /* real, ee/rpc/multitap/src/libmtap.c "#define MTAPSERV_PORT_CLOSE 0x80000902" - sibling service of SIF_SID_MTAP_PORT_OPEN above, not yet observed in any actual trace as of Round 1039 (listed here for completeness/citation only). */
#define SIF_SID_MTAP_GET_CONNECTION 0x80000903u /* real, ee/rpc/multitap/src/libmtap.c "#define MTAPSERV_GET_CONNECTION 0x80000903" - sibling service of SIF_SID_MTAP_PORT_OPEN above, not yet observed in any actual trace as of Round 1039 (listed here for completeness/citation only). */
#define SIF_SID_RMMAN2           0x80000C01u /* real, common/include/librm-common.h "#define RMMAN2_RPC_ID 0x80000C01" - PS2 Remote Manager (debug-station remote-control accessory) RPC service. Real EE client RMMan_Init() (ee/rpc/remote/src/librm.c) loops sceSifBindRpc() across RMMANX_RPC_ID(0x80000C02)/RMMAN2_RPC_ID(0x80000C01)/RMMAN_RPC_ID(0x80000C00) until any one binds, then issues exactly one sceSifCallRpc(&rmmanif, 0, 0, &buffer, 128, &buffer, 128, NULL, NULL) - note fno is always literally 0; the real command (RMMAN2_RPCFUNC_INIT=2) is encoded inside the 128-byte payload's own leading `cmd.command` word, not in the RPC fno. Discovered Round 1039, immediately downstream of the newly-fixed SIF_SID_MTAP_PORT_OPEN registration chain (0x8026FA40 dispatcher), as the next previously-uncatalogued real service the SCPH-50004 diskless boot reaches once binds are answered. */
#define SIF_SID_PAD_BIND_ID1_NEW 0x80000100u /* real, ee/rpc/pad/src/libpad.c "#define PAD_BIND_RPC_ID1_NEW 0x80000100" - the "new"-protocol PADMAN pad-bind RPC service (sibling of the already-handled SIF_SID_PAD_BIND_ID1_OLD/ID2_OLD 0x8000010F/0x8000011F above, which are the OLD-protocol bind IDs). padInit() races binding both PAD_BIND_RPC_ID1_NEW and PAD_BIND_RPC_ID1_OLD; if the NEW one wins, padInitialised=2 and a second bind is made to PAD_BIND_RPC_ID2_NEW (0x80000101, not yet observed/handled). padPortInit() then issues exactly one sceSifCallRpc(&padsif[0], 1, 0, &buffer, sizeof(buffer), &buffer, sizeof(buffer), NULL, NULL) - fno is always literally 1 (same "real command inside payload, not fno" pattern as SIF_SID_MTAP_PORT_OPEN/SIF_SID_RMMAN2 above); the real command is buffer.command=PAD_RPCCMD_INIT(0x10), matching this project's own trace ([R933EVT] call_sid=0x80000100 rpc_number=1). Real IOP-side handler (iop/input/padman/src/rpcserver.c RpcPadInit(), padCmds "BUILDING_XPADMAN" variant) calls data[3]=padInit((void*)data[4]) - data[3] is the 4th u32 word of the SAME buffer used for both send and receive (byte offset 12), matching the real client-side union's `padResult` member: struct{s32 unknown[3]; s32 result;} (ee/rpc/pad/src/libpad.c lines ~140-190, confirmed by direct read, NOT offset 4 like the MTAP/RMMAN2 replies above - a genuinely different real reply shape). Real IOP padInit() (iop/input/padman/src/padInit.c padInit()) always returns 1 on success and only returns 0 on internal CreateEventFlag/CreateThread resource-allocation failure - it does NOT check physical controller connectivity at all (that is discovered later via padPortOpen()/padGetState() polling, already handled by this project's existing PAD_BIND_ID1_OLD/ID2_OLD branches per Round 663/664's "real iop_sio2 pad-connected state" wiring). So the honest real reply here is result=1 (module init success), not a negative "no controller" error - unlike the MTAP/RMMAN2 fixes above, which really do represent absent-accessory replies. Discovered Round 1039 (2nd verification run, call=30), implemented Round 1040. */

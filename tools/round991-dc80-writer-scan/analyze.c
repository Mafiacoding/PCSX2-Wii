/* Round 991 (task #971, follow-up to task #970/Round 990): find the real
 * OSDSYS code path that is supposed to write the polled flag cell
 * 0x0040dc80 - Round 990 found the diskless SCPH-50004 boot's new
 * resting point (pc=0x0026fe9c) is a genuine `while (*0x0040dc80 == 0)
 * {}` busy-wait via a small array accessor at 0x0026fe90, and that the
 * cell (and 25 surrounding words) reads all-zero, i.e. never written in
 * this project's own boot trace.
 *
 * Method: boot to a point well past OSDSYS decompression completion
 * (Round 990 confirmed the resting loop, which lives inside already-
 * decompressed OSDSYS code, exists by instr=56M), then STATICALLY scan
 * the resident RAM image for the exact 2-instruction constant-
 * construction sequence the read-side accessor itself used to build the
 * 0x0040dc80 base address:
 *     lui  $r, 0x0041
 *     addiu $r, $r, -9088      ; low 16 bits = 0xdc80, sign-extended
 * (this is the canonical MIPS compiler pattern for any 32-bit constant
 * whose low half has its MSB set - since ADDIU sign-extends its
 * immediate, the upper half is pre-incremented by 1 to compensate; any
 * other function referencing the same table base/address, compiled by
 * the same toolchain, would use the identical 2-word encoding). Every
 * match is a real candidate site that TOUCHES this address (for read OR
 * write) - the following few instructions are dumped so each hit can be
 * classified as a load (matching Round 990's already-known read site)
 * or something new (a store - the actual candidate writer).
 *
 ...
 * 947/950/951/990's own analyze.c tools.
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
int main(int argc, char **argv)
{
    ...
    printf("[R991] reached_target_pc=%d ee_instr=%llu ee_pc=0x%08x halted=%d\n",
           reached_target, (unsigned long long)ee->instructions_executed, ee->pc, ee->halted);
    if (!reached_target) {
        fprintf(stderr, "[R991] did not reach the Round 990 resting pc within budget - aborting scan\n");
        return 1;
    }

    /* Static scan: whole 32MB RAM for the lui(0x0041)+addiu(-9088)
     * constant-construction pair used by the known read-side accessor.
     * Report every hit's address and disassemble the surrounding 8
     * instructions so each can be classified read vs write. */
    printf("\n[R991] Scanning full 32MB RAM for lui $r,0x0041 / addiu $r,$r,-9088 pairs (0x0040dc80 base construction)...\n");
    int hits = 0;
    for (uint32_t a = 0x00000000u; a < 0x02000000u; a += 4) {
        uint32_t w0 = ee_mem_read32(ee, a);
        uint32_t op0 = (w0 >> 26) & 0x3F;
        uint32_t rt0 = (w0 >> 16) & 0x1F;
        uint16_t imm0 = (uint16_t)(w0 & 0xFFFF);
        if (op0 != 0x0F || imm0 != 0x0041) continue; /* lui $rt0, 0x0041 */

        /* The addiu need not be immediately adjacent - compiler delay-
         * slot/scheduling can interleave an independent instruction
         * (Round 990's own accessor has "sll a0,a0,2" between its lui
         * and addiu). Search the next few instructions for a matching
         * addiu $r,$r,-9088 using the SAME register lui just set. */
        for (uint32_t off = 4; off <= 12; off += 4) {
            uint32_t w1 = ee_mem_read32(ee, a + off);
            uint32_t op1 = (w1 >> 26) & 0x3F;
            uint32_t rs1 = (w1 >> 21) & 0x1F;
            uint32_t rt1 = (w1 >> 16) & 0x1F;
            int16_t imm1 = (int16_t)(w1 & 0xFFFF);
            if (op1 != 0x09) continue; /* addiu */
            if (rs1 != rt0 || rt1 != rt0) continue; /* addiu $r,$r,imm, same reg as lui */
            if (imm1 != (int16_t)0xdc80) continue; /* -9088 */

            hits++;
            printf("\n[R991] HIT #%d: lui at 0x%08x, addiu at 0x%08x (register $%s):\n",
                   hits, a, a + off, reg_name(rt0));
            dump_range(ee, a, a + off + 24);
            break;
        }
    }
    printf("\n[R991] Total hits: %d\n", hits);

    printf("\n[R991] Wide context around HIT #1's registration site (0x0026fbe0-0x0026fce8):\n");
    dump_range(ee, 0x0026fbe0u, 0x0026fce8u);
